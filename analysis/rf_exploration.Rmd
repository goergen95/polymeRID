...
The modality of the data presented to RF is of higher importance here.
...
hold the number of trees constant at a value of 500. `mtry` effictvley specifies
the number of variables to look at each split within a tree. We took the square 
root of the number of variables devided by 3 as the first `mtry` value, the square
...
to noise. We can look at this more mathmatically by calculating the average 
...
the Savitzkiy-Golay filter applied to the raw and the normalised data followed
by the raw and the normalised data. The numbers can be interpreted as the loss in
