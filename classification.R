...
MODEL = "BASE"
TYPE = "FUSION" # either: FUSION, RFRAW, RFSG, CNND2, CNNND2
FORMAT = ".txt"
...
if (TYPE == "FUSION"){
  ...
  for (id in 1:length(ids)){
    ...
    class1 = samplePlot(data = data, sample = sample, class = classes[1],
                        probs = annotation, anno = ids[id])
    ...
    ggsave(plot=multiclass,file=paste0(plots,"/",ids[id],"_probClasses.png"),
           dpi=300,device="png",units="cm",width=50,height=30)
  }
...
if (TYPE == "RFRAW") {
  ...
  # predicting
  classRFRaw = as.character(stats::predict(rfModRaw, pcaRAW))
  probs =  stats::predict(rfModRaw, pcaRAW, type = "prob")
  pred = lapply(1:nrow(probs), function(x){
    which.max(probs[x,])
  })
  predVals = lapply(1:nrow(probs), function(x){
...
if (TYPE == "CNND2") {
  ...
  # predicting
  classCNND2 = as.character(classes[keras::predict_classes(cnnD2, x_sampleD2)+1])
  probs = as.data.frame(keras::predict_proba(cnnD2, x_sampleD2))
  names(probs)  = classes
...
if (TYPE == "CNNND2") {
  ...
  # predicting
  classCNNND2 = as.character(classes[keras::predict_classes(cnnND2, x_sampleND2)+1])
  probs = as.data.frame(keras::predict_proba(cnnND2, x_sampleND2))
  names(probs)  = classes
