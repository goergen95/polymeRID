...
rawPrimpke = rawPrimpke[which(rawPrimpke$Abbreviation %in%  c("PP","PE","PS","PA","HDPE","LDPE","PC","PES","PET","PVC")),]
...
refData$class[grep("Nylon",refData$class)] = "Nylon"
...
summary(data$class)
write.csv(data,file = paste0(ref,"reference_database.csv"))

classIndex = as.character(unique(data$class))

for (class in classIndex){
  tmp = data[data$class==class,]
  write.csv(tmp, file = paste0(ref,"reference_",class,".csv"))
}
